"""Flet Web 模式入口 —— 初始化 Agent / EventQueueManager, 启动 Flet Web UI

与 run_flet.py（桌面版）的区别：
  - 不依赖 flet_desktop
  - 使用 ft.app(target=main) 而非 ft.run()
  - page.window.* 调用做平台判断
"""
...
def main(page: ft.Page):
    """Flet Web 应用入口 —— 先检查本地 token，有效则跳过登录。"""
...
if __name__ == "__main__":
    ...
    ft.run(main, host="0.0.0.0", port=8502, view=ft.AppView.WEB_BROWSER, no_cdn=True)
