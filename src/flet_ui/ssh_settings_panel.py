...
class SshSettingsPanel:
    ...
    def _build(self):
        ...
        existing_section = ft.Column([
            ft.Divider(height=1, color=ft.Colors.GREY_300),
            ft.Text("📋 现有连接", size=14, weight=ft.FontWeight.W_600),
            self._conn_col,
    ...
    def _build_conn_rows(self) -> list[ft.Row]:
        """构建连接行列表，每行：信息 + 删除按钮"""
        rows = []
        for idx, c in enumerate(self._data.get("connections", [])):
            info_parts = [
            ...
            del_btn = ft.IconButton(
                ...
                icon_size=16,
                icon_color=ft.Colors.RED_400,
                tooltip=f"删除 {c.get('name', '?')}",
                data=idx,
                on_click=self._on_delete_conn_click,
            )
            rows.append(ft.Row([
                ft.Column([
    ...
    def _on_delete_conn_click(self, e):
        idx = e.control.data
        conn = self._data["connections"][idx]
        name = conn.get("name", "?")
        dlg = ft.AlertDialog(
            shape=ft.RoundedRectangleBorder(radius=3),
            title=ft.Text("确认删除", size=14),
            content=ft.Text(f"确定删除连接「{name}」？此操作不可撤销。", size=13),
            actions=[
                ft.TextButton(content="取消", on_click=lambda e: self.page.pop_dialog()),
                ft.ElevatedButton("删除",
                                  style=ft.ButtonStyle(color=ft.Colors.RED_400, shape=ft.RoundedRectangleBorder(radius=3)),
                                  on_click=lambda e, i=idx: self._do_delete_conn(i)),
            ],
        )
        self.page.show_dialog(dlg)

    def _do_delete_conn(self, idx):
        del self._data["connections"][idx]
        self._save()
        self.page.pop_dialog()
        self._refresh_conn_list()
