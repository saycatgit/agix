...
"""   
    ...
    def _build_dynamic_prompts(self):
        ...
        self.chat_only = (
            f"""
# 收到用户输入首要事情
- 用户输入信息后先要判断内容类型，如果用户信息可以分解为一个或多个有明确验收标准的工程任务的组合，直接用 start_task 启动任务模式。场景包括{ cats_with_desc }
- 如果用户信息和之前提交过的任务模式task相关联，继续用start_task启动任务模式。
- start_task提交任务后直接结束会话，禁止继续执行此任务。
- 如果用户需求涉及 2 个及以上 file_patch、跨文件修改、或多个独立操作步骤，但又不需要启动任务模式，必须先调用 update_plan 规划执行步骤，每个步骤完成后及时更新状态，直至任务完成。禁止跳过 update_plan 直接执行多步骤操作。
