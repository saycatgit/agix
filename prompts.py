...
- 用户输入信息后先要判断内容类型：
  ...
  - 如果用户信息可以分解为一个或多个有明确验收标准的相互独立的工程任务的组合，将符合（{ cats_with_desc }）场景包的任务内容提取出来作为 start_task输入启动任务模式。
  - 如果用户信息表明当前需求是之前某个项目的延续，直接用start_task启动任务模式，不要试图了解之前的项目信息，项目信息只在任务模式存档。
