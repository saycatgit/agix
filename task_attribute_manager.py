...
## 拆解原则（严格遵守）
- ⚠️ 子任务总数上限3个，非极其复杂的任务1个即可，禁止拆出第4个。
...
- 子任务若填写 history_task_index > 0，表示该子任务是对历史任务的延续、改进、修复或补充。
...
- history_task_index: 仅当前子任务为延续时填写（1-based），表示关联的历史主任务索引。
- history_subtask_index: 仅当前子任务为延续时填写，表示关联的历史子任务索引。
...
## 延续判断标准
- 新子任务与历史任务领域/主题相同或高度相关（如"继续开发"、"改进"、"修复"、"增加功能"等是对以往任务的补充或修改，可共用项目目录）→ 在对应子任务中填写 history_task_index
